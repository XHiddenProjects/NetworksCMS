            ...
            if($sql->setCredential($cred['server'],$cred['user'],$cred['psw'])){
                ...
                if(!empty($queries)){
                    if(isset($queries['type'])&&isset($queries['value'])){
                        if($db->selectData('topics',['*'],'WHERE '.strtolower($queries['type']).'="'.$queries['value'].'"')){
                            $data = $db->selectData('topics',isset($queries['selections']) ? array_filter(explode(',',$queries['selections']),function($i){return !preg_match('/psw|OnlineStat|public_key|ip/i',$i);}) : ['*'],'WHERE '.strtolower($queries['type']).'="'.$queries['value'].'"');
                        ...
                        $data = $db->selectData('topics',isset($queries['selections']) ? array_filter(explode(',',$queries['selections']),function($i){return !preg_match('/psw|OnlineStat|public_key|ip/i',$i);}) : ['*']);
                        return json_encode(['status'=>200,'success'=>$data??[]],JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES|JSON_NUMERIC_CHECK);
                    }
                }else $count = $db->selectData('topics',['*']);
